...
`Malformed` 与 `Remote` 的消息里若引用了对端的原始字节，会先转义并截断再拼进去，见第五节第 6 小节。
...
注意区分两类数据：`STAT` 行里的 `name` / `value` 是**要交给调用方使用**的内容，原样保留；
只有**错误消息**里的引用才做转义与截断。
...
空词那一类不论成因（整段为空交给长度判据，片段为空、首空格、尾空格、连续两个空格交给扫描）
都收敛到**同一条** `stats section has an empty word`，错误种类不会随空词出现的位置变化。
...
- 校验：`validate_key` 对 `0x1F`、`0x7F` 等控制字符与长度边界的判定（含多字节字符按
  ...
  首尾空格、连续两个空格、含制表符的段名都拒绝，其中空词的四种成因（空段名走长度判据，其余
  走扫描）收敛到同一条消息，超长时报出实际字节数与 `1..250`，非法字节则被转义后随段名带回。
